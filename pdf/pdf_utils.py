...
class MyPDFHandler(object):
    ...
    def save2file(self,new_file_name):
        ...
        print('Generated file successfully, new pdf file path is: {0}'.format(new_file_name))
    ...
    def add_bookmarks(self,bookmarks):
        ...
        :return: None
        '''
        bookmark_object_list = []
        for title,page,level in bookmarks:
            parent = None
            if level != 0:
                for parent_ref,parent_level in reversed(bookmark_object_list):
                    if parent_level<level:
                        parent = parent_ref 
                        break

            current = self.add_one_bookmark(title,page,parent=parent) 
            bookmark_object_list.append((current,level))

        print('Add bookmarks successfully, there are total {0} items added.'.format(len(bookmarks)))
            ...
            for line in fin:
                ...
                if title and page:
                    try:
                        ...
                        bookmarks.append((title, page,self.count_tab(title)))
                    except ValueError as msg:
                        print(msg)
        print('Loaded all bookmarks: {0} items.'.format(len(bookmarks)))
        return bookmarks
    ...
    '''
    统计标题里面的\t字符数，以确定当前标题的level，仅从标题开始统计
    '''
